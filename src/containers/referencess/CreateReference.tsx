...
import { Button, Form, Input, FormFeedback, Row, Col } from 'reactstrap';
import * as yup from 'yup';
import { useCreateReferenceMutation } from '../../generated/graphql';

const CreateReference = () => {
  const { register, handleSubmit, reset, errors } = useForm<CreateReferenceFormValues>({ validationSchema })
    ...
    >
      <Row>
        ...
        <Col lg={2} className="mt-3 mt-lg-0">
          <Button block type="submit" className="pwc" disabled={createReferenceM.loading}>
            Add
          </Button>
        </Col>
      </Row>
...
const validationSchema = yup.object().shape({
  name: yup.string().required().test('reference', 'Require a hashtag', function (value: string) {
    return (value[0] === '#')
  })
})